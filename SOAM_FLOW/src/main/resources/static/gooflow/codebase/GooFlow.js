 ...
 * GooGlow修改日志
 * 修改人：zhouyanping
 * 修改内容：1.将$lineDom.M 属性改成小学"m"，便于后台服务处理数据。
 * 			2.将样式图标 node 和 task互换。
 * 			3.增加节点状态属性，支持不同状态展示不同背景色。
 * 
 * 说明：！！后续版本修改以此为基础。
 */
...
GooFlow.prototype = {
    ...
    initDraw: function (id, width, height) {
        ...
        if (GooFlow.prototype.useSVG != "") {
             ...
             * 修改日志
             * 修改人：zhouyanping
             * 修改时间：2016年12月26日14:48:00
             * 修改内容：新增节点不同状态svg画图箭头
             */
            defs.appendChild(GooFlow.prototype.getSvgMarker("status_unstart", GooFlow.prototype.status.unstart));
            defs.appendChild(GooFlow.prototype.getSvgMarker("status_running", GooFlow.prototype.status.running));
            defs.appendChild(GooFlow.prototype.getSvgMarker("status_end", GooFlow.prototype.status.end));
            defs.appendChild(GooFlow.prototype.getSvgMarker("status_error", GooFlow.prototype.status.error));
        }
        else {
    ...
    addNode: function (id, json) {
         ...
         * 修改日志
         * 修改人：zhouyanping
         * 修改时间：2016年12月26日10:20:43
         * 修改内容：判断节点状态，设置状态颜色
         */
        if (json.hasOwnProperty("status") && json.status!="") {
            var statusColor = GooFlow.prototype.getNodestatusColor(json).statusColor;
            this.$nodeDom[id].css({
     ...
     * 修改日志
     * 修改人：zhouyanping
     * 修改时间：2016年12月26日11:23:18
     * 修改内容：新增从初始化数据中获取 //获取结点/连线/分组区域的详细信息
     * @type {string}
     */
    getItemInfoToinitData: function (id, type) {
        switch (type) {
            case "node":
                return GooFlow.prototype.initData.nodes[id] || null;
            case "line":
                return GooFlow.prototype.initData.lines[id] || null;
            case "area":
                return GooFlow.prototype.initData.areas[id] || null;
        }
    },
    //获取结点/连线/分组区域的详细信息
    ...
    loadData: function (data) {
         ...
         * 修改日志
         * 修改人：zhouyanping
         * 修改时间：2016年12月26日11:20:57
         * 修改内容：新增记录初始化数据，方便使用
         * @type {string}
         */
        GooFlow.prototype.initData = data;
...
///////////以下为有关画线的方法
    ...
    drawLine: function (id, sp, ep, mark, dash) {
        ...
        if (GooFlow.prototype.useSVG != "") {
             ...
             * 修改日志
             * 修改人：zhouyanping
             * 修改时间：2016年12月26日14:29:55
             * 修改内容：判断节点状态，设置状态颜色
             */
            var tempLine = this.getItemInfoToinitData(id, "line");
            if (tempLine!=null && tempLine.hasOwnProperty("status") && tempLine.status!="") {
                var statusObj = GooFlow.prototype.getNodestatusColor(tempLine);
                path.attributes["stroke"].value = statusObj.statusColor;
                path.attributes["marker-end"].value = "url(#" + statusObj.statusDefs + ")";
            }
        } else {
             ...
             * 修改日志
             * 修改人：zhouyanping
             * 修改时间：2016年12月26日14:30:20
             * 修改内容：判断节点状态，设置状态颜色
             */
            var tempLine = this.getItemInfoToinitData(id, "line");
            if (tempLine!=null && tempLine.hasOwnProperty("status") && tempLine.status!="") {
                var statusObj = GooFlow.prototype.getNodestatusColor(tempLine);
                line.strokeColor = statusObj.statusColor;
                line.fillColor = "url(#" + statusObj.statusDefs + ")";
            }
        }
        return line;
    },
    ...
    drawPoly: function (id, sp, m1, m2, ep, mark) {
        ...
        if (GooFlow.prototype.useSVG != "") {
             ...
             * 修改日志
             * 修改人：zhouyanping
             * 修改时间：2016年12月26日11:29:43
             * 修改内容：判断节点状态，设置状态颜色
             */
            var tempLine = this.getItemInfoToinitData(id, "line");
            if (tempLine!=null && tempLine.hasOwnProperty("status") && tempLine.status!="") {
                var statusObj = GooFlow.prototype.getNodestatusColor(tempLine);
                poly.strokeColor = statusObj.statusColor;
                poly.fillColor = "url(#" + statusObj.statusDefs + ")";
            }
        }
        return poly;
    },
 ...
 * 修改日志
 * 修改人：zhouyanping
 * 修改时间：2016年12月26日12:38:14
 * 修改内容：新增节点状态对象，用于存储不同状态颜色，初始化设置默认值，如需自定义颜色可在GooFlow控件外重置即可。
 * @type {{end: string, running: string, error: string}}
 */
 ...
 * 修改日志
 * 修改人：zhouyanping
 * 修改时间：2016年12月26日10:19:43
 * 修改内容：新增  传入节点对象，获取节点状态颜色
 * @param node  节点
 */
GooFlow.prototype.getNodestatusColor = function (node) {
    var statusObj = {statusColor: null, statusDefs: null};
    if (node.hasOwnProperty("status") && node.status!="") {
        switch (node.status) {
            case 'unstart':
                statusObj.statusColor = GooFlow.prototype.status.unstart;
                statusObj.statusDefs = "status_unstart";
                break;
            case 'running':
                statusObj.statusColor = GooFlow.prototype.status.running;
                statusObj.statusDefs = "status_running";
                break;
            case 'end':
                statusObj.statusColor = GooFlow.prototype.status.end;
                statusObj.statusDefs = "status_end";
                break;
            case 'error':
                statusObj.statusColor = GooFlow.prototype.status.error;
                statusObj.statusDefs = "status_error";
                break;
        }
    }
    return statusObj;
